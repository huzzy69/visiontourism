...
export const AboutSection: React.FC = () => {
  return (
    <section className="bg-brand-cream-200 py-20 border-t border-b border-brand-cream-300 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          
          {/* Visual Collages panel (Left Column) */}
          <div className="relative flex justify-center items-center">
            {/* Main Image */}
            <div className="w-[85%] aspect-[4/3] rounded-sm overflow-hidden shadow-2xl border-4 border-white z-10 relative">
              <img
                src="https://images.unsplash.com/photo-1475503572774-15a45e5d60b9?q=80&w=1000&auto=format&fit=crop"
                alt="Highlands Castle scenery"
                className="w-full h-full object-cover"
              />
            </div>
            
            {/* Overlapping secondary image */}
            <div className="absolute -bottom-6 -left-4 w-[50%] aspect-[3/4] rounded-sm overflow-hidden shadow-2xl border-4 border-white z-20 hidden sm:block">
              <img
                src="https://images.unsplash.com/photo-1543872084-c7bd3822856f?q=80&w=600&auto=format&fit=crop"
                alt="Cotswolds village street"
                className="w-full h-full object-cover"
              />
            </div>

            {/* Accent gold box behind */}
            <div className="absolute -top-6 -right-4 w-[40%] h-[80%] bg-brand-gold-400/20 rounded-sm pointer-events-none z-0" />
          </div>

          {/* Text Story panel (Right Column) */}
          <div className="flex flex-col gap-6">
            <div>
              <span className="text-brand-gold-500 font-semibold tracking-[0.2em] text-xs uppercase block mb-1">
                Heritage & Standards
              </span>
              <h2 className="font-serif text-3xl sm:text-4xl text-brand-green-900 font-bold tracking-wide">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-brand-cream-300 pt-6 mt-2">
              <div className="flex gap-2">
                <ShieldCheck className="w-5 h-5 text-brand-gold-400 flex-shrink-0" />
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-brand-green-900">Safety First</span>
                  <span className="text-[10px] text-slate-500 mt-0.5">Licensed operators</span>
                </div>
              </div>
              <div className="flex gap-2">
                <Compass className="w-5 h-5 text-brand-gold-400 flex-shrink-0" />
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-brand-green-900">Local Guides</span>
                  <span className="text-[10px] text-slate-500 mt-0.5">Rich storytelling</span>
                </div>
              </div>
              <div className="flex gap-2">
                <Milestone className="w-5 h-5 text-brand-gold-400 flex-shrink-0" />
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-brand-green-900">Flexibility</span>
